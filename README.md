...
- Next step for both open items above: independently source Solana price data for the false-positive tokens and for the full true-positive set, rather than relying on the bot's own second buy as a proxy. The competition rules explicitly permit querying public Solana on-chain data as External Data, provided it is not used to alter the decision-time features in Parts 2–3 [6]
